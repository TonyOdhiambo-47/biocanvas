...
const step1: Step = {
  ...
  render({ ctx, w, h, t }) {
    ...
    const codonW = 54;
    const gap = 6;
    const total = codons.length * (codonW + gap) - gap;
    const x = (w - total) / 2;
    const y = h / 2 + 20;
    ctx.save();
    ctx.globalAlpha = op;
    drawMRna(ctx, x, y, codons, 0, { showCap: true });
    ctx.restore();
...
const step4: Step = {
  ...
  render({ ctx, w, h, t }) {
    ...
    const x = (w - total) / 2;
    const y = h / 2 + 60;
    drawMRna(ctx, x, y, codons, 1, { showCap: true });
...
const step6: Step = {
  ...
  render({ ctx, w, h, t }) {
    ...
    drawMRna(ctx, x, y, codons, 2, { showCap: true });
...
const step7: Step = {
  ...
  render({ ctx, w, h, t }) {
    ...
    const x = (w - total) / 2;
    const y = h / 2 + 60;
    drawMRna(ctx, x, y, codons, codons.length - 1, { showCap: true });
